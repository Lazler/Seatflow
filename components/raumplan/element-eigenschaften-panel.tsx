...
export default function ElementEigenschaftenPanel({ el, kategorien, fremdeSitzIds, onChange, onLoeschen, onSchliessen, onDuplizieren }: Props) {
  ...
  const typLabel = el.typ === "stehplatz" ? t.elementPanel.stehplatzZone : t.editorToolbar.elementTypen[el.typ];
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden animate-slide-up">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {el.typ === "reihe" && (<>
          <Stepper label={t.elementPanel.sitze} value={el.anzahlSitze} min={1} max={60}
            onChange={(v) => onChange({ anzahlSitze: v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.sitzabstand} value={el.sitzAbstand} min={24} max={60} einheit="px"
            onChange={(v) => onChange({ sitzAbstand: v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.ersteSitznummer} value={el.nummerStart ?? 1} min={1} max={200}
            onChange={(v) => onChange({ nummerStart: v === 1 ? undefined : v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.kruemmung} value={el.bogen ?? 0} min={0} max={80} schritt={4} einheit="px"
            onChange={(v) => onChange({ bogen: v === 0 ? undefined : v } as Partial<SitzplanElement>)} />
          <ToggleRow label={t.elementPanel.nummerierungVonRechts} value={el.nummerRichtung === "rtl"}
            onChange={(v) => onChange({ nummerRichtung: v ? "rtl" : undefined } as Partial<SitzplanElement>)} />
        </>)}

        {el.typ === "tischreihe" && (<>
          <Stepper label={t.elementPanel.sitzeProSeite} value={el.sitzeProSeite} min={1} max={12}
            onChange={(v) => onChange({ sitzeProSeite: v } as Partial<SitzplanElement>)} />
          <ToggleRow label={t.elementPanel.sitzreiheOben} value={el.sitzeOben}
            onChange={(v) => onChange({ sitzeOben: v } as Partial<SitzplanElement>)} />
          <ToggleRow label={t.elementPanel.sitzreiheUnten} value={el.sitzeUnten}
            onChange={(v) => onChange({ sitzeUnten: v } as Partial<SitzplanElement>)} />
        </>)}

        {el.typ === "rundtisch" && (<>
          <Stepper label={t.elementPanel.sitze} value={el.anzahlSitze} min={2} max={20}
            onChange={(v) => onChange({ anzahlSitze: v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.radius} value={el.tischRadius} min={20} max={120} einheit="px"
            onChange={(v) => onChange({ tischRadius: v } as Partial<SitzplanElement>)} />
        </>)}

        {el.typ === "stehplatz" && (<>
          <Stepper label={t.elementPanel.kapazitaet} value={el.kapazitaet} min={1} max={500} schritt={5}
            onChange={(v) => onChange({ kapazitaet: v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.breite} value={el.breite} min={80} max={1200} schritt={20} einheit="px"
            onChange={(v) => onChange({ breite: v } as Partial<SitzplanElement>)} />
          <Stepper label={t.elementPanel.hoehe} value={el.hoehe} min={60} max={900} schritt={20} einheit="px"
        ...
        {el.typ === "text" && (<>
          <div className="px-4 py-2 space-y-1">
            <input
              ...
              placeholder={t.elementPanel.textPlaceholder}
            />
          </div>
          <Stepper label={t.elementPanel.schriftgroesse} value={el.fontSize} min={10} max={48} schritt={2} einheit="px"
        ...
        {el.typ !== "text" && el.typ !== "stehplatz" && (<>
          <SectionLabel>{t.elementPanel.anzeige}</SectionLabel>
          <ToggleRow label={t.elementPanel.sitznummernAnzeigen} value={!(el.nummerAusblenden ?? false)}
        ...
        {el.typ !== "text" && (<>
        <SectionLabel>{t.elementPanel.preiskategorie}</SectionLabel>
            ...
            return (
              <button key={k.id} type="button"
                ...
                <span className={`text-xs font-normal ${aktiv ? "opacity-80" : "text-muted-foreground"}`}>
                  {(k.preis_cent / 100).toLocaleString(intlLocale(locale), { style: "currency", currency: "EUR" })}
                </span>
      ...
      <div className="shrink-0 px-4 py-3 border-t border-border space-y-2">
        <button type="button" onClick={onDuplizieren}
          ...
          <Copy className="h-3.5 w-3.5" /> {t.elementPanel.elementDuplizieren}
        </button>
        {loeschen ? (
          <div className="rounded-lg border border-destructive/30 p-3 space-y-2 animate-slide-up"
            ...
            <p className="text-sm font-medium text-destructive">{t.elementPanel.wirklichLoeschen}</p>
            <p className="text-xs text-muted-foreground">
              {fmt(sitzAnzahl === 1 ? t.elementPanel.loeschenHinweisSg : t.elementPanel.loeschenHinweisPl, { typ: typLabel, bez: el.bezeichnung, n: sitzAnzahl })}
            </p>
            <div className="flex gap-2">
              <Button size="sm" variant="destructive" className="flex-1" onClick={onLoeschen}>
                <Trash2 className="h-3.5 w-3.5 mr-1.5" /> {t.elementPanel.loeschen}
              </Button>
              <Button size="sm" variant="outline" onClick={() => setLoeschen(false)}>
                {t.elementPanel.abbrechen}
              </Button>
            </div>
          </div>
        ) : (
