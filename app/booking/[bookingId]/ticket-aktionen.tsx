...
export default function TicketAktionen({ buchungId, sprache = "de" }: { buchungId: string; sprache?: BuchungsSprache }) {
  ...
  async function erneutSenden() {
    setSendet(true);
    const res = await fetch(`/api/booking/${buchungId}/resend`, { method: "POST" });
    setSendet(false);
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-col sm:flex-row gap-2">
        <a href={`/api/booking/${buchungId}/pdf`} className={btn}>
        ...
        <a href={`/api/booking/${buchungId}/calendar`} className={btn}>
